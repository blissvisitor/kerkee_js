...
var destPath = outputPath + "/target";
...
gulp.task('jshint', function () {
    return gulp.src([devPath + '/modules/**/*.js', devPath + '/coreApi/**/*.js'])
        .pipe(plugins.jshint())
...
gulp.task('cleanTemp', function () {
    return gulp.src([tempPath, destPath + "/*.txt", destPath + "/less", destPath + "/tpl", destPath + "/tmod", destPath + "/lib/underscore.js", destPath + "/build", basePath + "/bin", destPath + "/modules/liveonline"])
        .pipe(plugins.clean());
});
 ...
 * @dest destPath
 */
gulp.task('rjsCompression', ["cleanTarget"], function () {
    var deferred = Q.defer();

    var spawn = require('child_process').spawn;
    if(!util.env.web){
        var run = spawn('sh', ['-m', corePath + '/build/rjs.sh', 'client']);
    }else{
        var run = spawn('sh', ['-m', corePath + '/build/rjs.sh', 'web']);
    }
 ...
 */
gulp.task('build', ["compressionzip"], function () {
    var deferred = Q.defer();
    deferred.resolve();
...
gulp.task('md5',['build'],function(){
    var spawn = require('child_process').spawn;
    var run = spawn('md5', [outputPath + '/bin/html_' + globalTimesup + '.zip']);
    run.stdout.on('data', function (data) {
        console.log('stdout: ' + data);
    });
    run.stderr.on('data', function (data) {
...
var appData = {};   // 要创建的页面模块数据，通过命令行参数获取
var isExists = true;   // 要创建的页面模块是否已经存在
...
gulp.task('includeTpl',function () {
    ...
    // 判断要创建的页面模块是否已经存在，避免覆盖
    isExists = fs.existsSync(devPath + '/modules/' + appName);
    ...
    if(!isExists){
        ...
        return gulp.src([
                corePath + '/tmod/app/app.tpl',
                corePath + '/tmod/app/appDo.tpl',
                corePath + '/tmod/app/app.html',
                corePath + '/tmod/app/appApi.tpl',
                corePath + '/tmod/app/appapiInterFace.tpl'
            ])
            .pipe(plugins.fileInclude({
                prefix: '@@',
                basepath: '@file'
            }))
            .pipe(gulp.dest(corePath + '/tmod/app/dest'))
            .pipe(createLog());
    }else{
        console.log('要创建的页面模块已经存在');
    }
...
gulp.task('resolveTpl',["includeTpl"],function () {
    if(!isExists){
        return gulp.src([
                corePath + '/tmod/app/dest/app.tpl',
                corePath + '/tmod/app/dest/appDo.tpl',
                corePath + '/tmod/app/dest/app.html',
                corePath + '/tmod/app/dest/appApi.tpl',
                corePath + '/tmod/app/dest/appapiInterFace.tpl'
            ])
            .pipe(plugins.data(function () {
...
gulp.task('createapp', ["resolveTpl"], function () {
    if(!isExists){
        ...
        gulp.src(corePath + '/tmod/app/dest/app.tpl')
            .pipe(plugins.rename({
                basename: appData.app,
                extname: ".js"
            }))
            .pipe(gulp.dest(devPath + '/modules/'+appData.app))
            .pipe(createLog());

        // 创建部署业务逻辑js文件，如 indexDo.js
        gulp.src(corePath + '/tmod/app/dest/appDo.tpl')
            .pipe(plugins.rename({
        ...
        gulp.src([corePath + '/tmod/app/dest/*.html'])
            .pipe(plugins.rename({
                basename: appData.app,
                extname: ".html"
            }))
        ...
        gulp.src(corePath + '/tmod/app/dest/appApi.tpl')
            .pipe(plugins.rename({
                basename: appData.app + 'Api',
                extname: ".js"
            }))
            .pipe(gulp.dest(devPath + '/clientApi'))
            .pipe(createLog());

        // 创建部署跨平台接口js文件，如 indexapiInterFace.js
        gulp.src(corePath + '/tmod/app/dest/appapiInterFace.tpl')
            .pipe(plugins.rename({
                basename: appData.app + 'apiInterFace',
                extname: ".js"
            }))
            .pipe(gulp.dest(devPath + '/crossplatform/interface'))
            .pipe(createLog());
    }
});
